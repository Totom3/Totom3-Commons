 ...
 * <p>
 * @DarkBlade12 and allows you to display all Minecraft particle effects on a
 * Bukkit server
 * <p>
 * You are welcome to use it, modify it and redistribute it under the following
 * conditions: <ul> <li>Don't claim this class as your own
 * <li>Don't remove this disclaimer </ul>
 * <p>
 * Special thanks: <ul>
 * <li>@microgeek (original idea, names and packet parameters) <li>@ShadyPotato
 * (1.8 names, ids and packet parameters) <li>@RingOfStorms (specific particle
 * direction) </ul>
 * <p>
...
public enum ParticleEffect {
    ...
    /**
     * A particle effect which is displayed by exploding tnt and creepers: <ul>
     * <li>It looks like a white cloud <li>The speed value influences the
     * velocity at which the particle flies off </ul>
     */
    EXPLOSION_NORMAL("explode", 0, -1),

    /**
     * A particle effect which is displayed by exploding ghast fireballs and
     * wither skulls: <ul> <li>It looks like a gray ball which is fading away
    ...
    /**
     * A particle effect which is displayed by exploding tnt and creepers: <ul>
     * <li>It looks like a crowd of gray balls which are fading away <li>The
     * speed value has no influence on this particle effect </ul>
     */
    EXPLOSION_HUGE("hugeexplosion", 2, -1),

    /**
     * A particle effect which is displayed by launching fireworks: <ul> <li>It
     * looks like a white star which is sparkling <li>The speed value influences
     * the velocity at which the particle flies off </ul>
     */
    FIREWORKS_SPARK("fireworksSpark", 3, -1),

    /**
     * A particle effect which is displayed by swimming entities and arrows in
     * water: <ul> <li>It looks like a bubble <li>The speed value influences the
     * velocity at which the particle flies off </ul>
     */
     ...
     * wolves: <ul> <li>It looks like a blue drop <li>The speed value has no
     * influence on this particle effect </ul>
     */
    WATER_SPLASH("splash", 5, -1),

    /**
     * A particle effect which is displayed on water when fishing: <ul> <li>It
     * looks like a blue droplet <li>The speed value influences the velocity at
     * which the particle flies off </ul>
     */
    WATER_WAKE("wake", 6, 7),

    /**
     * A particle effect which is displayed by water: <ul> <li>It looks like a
     * tiny blue square <li>The speed value has no influence on this particle
     * effect </ul>
     */
     ...
     * in the void: <ul> <li>It looks like a tiny gray square <li>The speed
     * value has no influence on this particle effect </ul>
     */
    SUSPENDED_DEPTH("depthSuspend", 8, -1),

    /**
     * A particle effect which is displayed when landing a critical hit and by
     * arrows: <ul> <li>It looks like a light brown cross <li>The speed value
     * influences the velocity at which the particle flies off </ul>
     */
    CRIT("crit", 9, -1),

    /**
     * A particle effect which is displayed when landing a hit with an enchanted
     * weapon: <ul> <li>It looks like a cyan star <li>The speed value influences
     * the velocity at which the particle flies off </ul>
     */
     ...
     * dispensers, end portals, brewing stands and monster spawners: <ul> <li>It
     * looks like a little gray cloud <li>The speed value influences the
     * velocity at which the particle flies off </ul>
     */
     ...
     * blazes: <ul> <li>It looks like a large gray cloud <li>The speed value
     * influences the velocity at which the particle flies off </ul>
     */
     ...
     * enchanting hit something: <ul> <li>It looks like a white swirl <li>The
     * speed value causes the particle to only move upwards when set to 0 </ul>
     */
    SPELL("spell", 13, -1),

    /**
     * A particle effect which is displayed when instant splash potions hit
     * something: <ul> <li>It looks like a white cross <li>The speed value
     * causes the particle to only move upwards when set to 0 </ul>
     */
    SPELL_INSTANT("instantSpell", 14, -1),

    /**
     * A particle effect which is displayed by entities with active potion
     * effects: <ul> <li>It looks like a colored swirl <li>The speed value
     * causes the particle to be colored black when set to 0 <li>The particle
     * color gets lighter when increasing the speed and darker when decreasing
     * the speed </ul>
     */
    ...
    /**
     * A particle effect which is displayed by entities with active potion
     * effects applied through a beacon: <ul> <li>It looks like a transparent
     * colored swirl <li>The speed value causes the particle to be always
     * colored black when set to 0 <li>The particle color gets lighter when
     * increasing the speed and darker when decreasing the speed </ul>
     */
    SPELL_MOB_AMBIENT("mobSpellAmbient", 16, -1),

    /**
     * A particle effect which is displayed by witches: <ul> <li>It looks like a
     * purple cross <li>The speed value causes the particle to only move upwards
     * when set to 0 </ul>
     */
     ...
     * <ul> <li>It looks like a blue drip <li>The speed value has no influence
     * on this particle effect </ul>
     */
    DRIP_WATER("dripWater", 18, -1),

    /**
     * A particle effect which is displayed by blocks beneath a lava source:
     * <ul> <li>It looks like an orange drip <li>The speed value has no
     * influence on this particle effect </ul>
     */
     ...
     * village: <ul> <li>It looks like a cracked gray heart <li>The speed value
     * has no influence on this particle effect </ul>
     */
    VILLAGER_ANGRY("angryVillager", 20, -1),

    /**
     * A particle effect which is displayed when using bone meal and trading
     * with a villager in a village: <ul> <li>It looks like a green star <li>The
     * speed value has no influence on this particle effect </ul>
     */
    VILLAGER_HAPPY("happyVillager", 21, -1),

    /**
     * A particle effect which is displayed by mycelium: <ul> <li>It looks like
     * a tiny gray square <li>The speed value has no influence on this particle
     * effect </ul>
     */
    TOWN_AURA("townaura", 22, -1),

    /**
     * A particle effect which is displayed by note blocks: <ul> <li>It looks
     * like a colored note <li>The speed value causes the particle to be colored
     * green when set to 0 </ul>
     */
    NOTE("note", 23, -1),

    /**
     * A particle effect which is displayed by nether portals, endermen, ender
     * pearls, eyes of ender, ender chests and dragon eggs: <ul> <li>It looks
     * like a purple cloud <li>The speed value influences the spread of this
     * particle effect </ul>
     */
    PORTAL("portal", 24, -1),
     ...
     * nearby bookshelves: <ul> <li>It looks like a cryptic white letter <li>The
     * speed value influences the spread of this particle effect </ul>
     */
    ENCHANTMENT_TABLE("enchantmenttable", 25, -1),

    /**
     * A particle effect which is displayed by torches, active furnaces, magma
     * cubes and monster spawners: <ul> <li>It looks like a tiny flame <li>The
     * speed value influences the velocity at which the particle flies off </ul>
     */
    FLAME("flame", 26, -1),

    /**
     * A particle effect which is displayed by lava: <ul> <li>It looks like a
     * spark <li>The speed value has no influence on this particle effect </ul>
     */
    LAVA("lava", 27, -1),

    /**
     * A particle effect which is currently unused: <ul> <li>It looks like a
     * transparent gray square <li>The speed value has no influence on this
     * particle effect </ul>
     */
    FOOTSTEP("footstep", 28, -1),

    /**
     * A particle effect which is displayed when a mob dies: <ul> <li>It looks
     * like a large white cloud <li>The speed value influences the velocity at
     * which the particle flies off </ul>
     */
    CLOUD("cloud", 29, -1),
     ...
     * redstone torches and redstone repeaters: <ul> <li>It looks like a tiny
     * colored cloud <li>The speed value causes the particle to be colored red
     * when set to 0 </ul>
     */
    REDSTONE("reddust", 30, -1),

    /**
     * A particle effect which is displayed when snowballs hit a block: <ul>
     * <li>It looks like a little piece with the snowball texture <li>The speed
     * value has no influence on this particle effect </ul>
     */
    ...
    /**
     * A particle effect which is currently unused: <ul> <li>It looks like a
     * tiny white cloud <li>The speed value influences the velocity at which the
     * particle flies off </ul>
     */
    SNOW_SHOVEL("snowshovel", 32, -1),

    /**
     * A particle effect which is displayed by slimes: <ul> <li>It looks like a
     * tiny part of the slimeball icon <li>The speed value has no influence on
     * this particle effect </ul>
     */
    SLIME("slime", 33, -1),

    /**
     * A particle effect which is displayed when breeding and taming animals:
     * <ul> <li>It looks like a red heart <li>The speed value has no influence
     * on this particle effect </ul>
     */
    HEART("heart", 34, -1),

    /**
     * A particle effect which is displayed by barriers: <ul> <li>It looks like
     * a red box with a slash through it <li>The speed value has no influence on
     * this particle effect </ul>
     */
     ...
     * block: <ul> <li>It looks like a little piece with an item texture </ul>
     */
    ITEM_CRACK("iconcrack", 36, -1, true),

    /**
     * A particle effect which is displayed when breaking blocks or sprinting:
     * <ul> <li>It looks like a little piece with a block texture <li>The speed
     * value has no influence on this particle effect </ul>
     */
    BLOCK_CRACK("blockcrack", 37, -1, true),

    /**
     * A particle effect which is displayed when falling: <ul> <li>It looks like
     * a little piece with a block texture </ul>
     */
    BLOCK_DUST("blockdust", 38, 7, true),

    /**
     * A particle effect which is displayed when rain hits the ground: <ul>
     * <li>It looks like a blue droplet <li>The speed value has no influence on
     * this particle effect </ul>
     */
    WATER_DROP("droplet", 39, 8),

    /**
     * A particle effect which is currently unused: <ul> <li>It has no visual
     * effect </ul>
     */
    ITEM_TAKE("take", 40, 8),

    /**
     * A particle effect which is displayed by elder guardians: <ul> <li>It
     * looks like the shape of the elder guardian <li>The speed value has no
     * influence on this particle effect </ul>
     */
    ...
    /**
     * Determine if the distance between
     * <p>
     * @param location and one of the players exceeds 256
     * <p>
     * @param location Location to check
     * <p>
     ...
     * @throws ParticleDataException    If the particle effect requires
     *                                  additional data
	...
	if (!isSupported()) {
	    throw new ParticleVersionException(notSupported());
	}
	if (requiresData) {
     ...
     * visible for all players within a certain range in the world of
     * <p>
     *
	...
	return "The particle " + name + " is not supported by your server version (required: " + requiredVersion + ", actual: " + ParticlePacket.getVersion() + ")";
    }
	...
	private static final long serialVersionUID = 320_3085_3871_6073_7484L;
	 ...
	 */
	ParticleDataException(String message) {
	    super(message);
	}
    }
	 ...
	 * @param cause   Cause of the exception
	 */
	ParticleVersionException(String message) {
	    super(message);
	}
    }
	...
	public static void initialize() throws VersionIncompatibleException {
	    ...
	    } catch (NumberFormatException | ReflectiveOperationException | SecurityException exception) {
	 ...
	 *                                      unknown error
	 * @throws PacketSendingException       if sending fails due to an
	 *                                      unknown error
	 */
	 ...
	 *               range for particles is usually 16, but it can differ
	 *               for some types)
	 * <p>
